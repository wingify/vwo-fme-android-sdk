...
- Added Force Users support for Rollout and Personalize rules. Users forced on receive the rule experience regardless of audience targeting or traffic percentage. Users forced off (`not` in `whitelistedSegments`) are excluded from that rule and do not fall through to audience or traffic.

## [1.65.0] - 2026-08-26

### Added

- SDK init options (`initConfig`) are now sent on the usage stats event so the app dashboard can read them.
