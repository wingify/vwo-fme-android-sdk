 ...
 */
package com.wingify.packages.network_layer.client

import com.google.gson.Gson
import com.wingify.ServiceContainer
import com.wingify.WingifyClient
import com.wingify.constants.Constants
import com.wingify.constants.Constants.HTTP_STATUS_CODE_400
import com.wingify.constants.Constants.HTTP_STATUS_CODE_401
import com.wingify.constants.Constants.MAX_RETRY_ATTEMPTS
import com.wingify.constants.Constants.RETRY_DELAY
import com.wingify.constants.Constants.SETTINGS_ENDPOINT
import com.wingify.constants.Constants.SETTINGS_MAX_RETRY_ATTEMPTS
...
import com.wingify.utils.NetworkUtil.Companion.removeNullValues
...
class NetworkClient : NetworkClientInterface {
    ...
    override fun GET(request: RequestModel, serviceContainer: ServiceContainer?): ResponseModel {
        return retryWrapper(
            ...
            try {
                ...
                connection.requestMethod = HttpMethods.GET.value
    ...
    override fun POST(request: RequestModel, serviceContainer: ServiceContainer?): ResponseModel {
        return retryWrapper(
            ...
            try {
                ...
                val connection = url.openConnection() as HttpURLConnection
                connection.requestMethod = HttpMethods.POST.value
                connection.doOutput = true

                val body = networkOptions["body"]
                bodyArray = if (body is LinkedHashMap<*, *>) {
                    val jsonBody: String = WingifyClient.objectMapper.writeValueAsString(body)
    ...
    private fun createNetWorkAndRetryDebugEvent(
        ...
        return try {
            ...
            val lt =
                if (response.error == null) LogLevelEnum.INFO.name else com.wingify.enums.LogLevelEnum.ERROR.name
