...
import com.wingify.packages.storage.LocalStorageController
import com.wingify.providers.StorageProvider
import com.wingify.services.AliasApiService
...
class AliasIdentityManager(val serviceContainer: ServiceContainer) {
     ...
     */
    fun maybeGetAliasAwareUserIdSync(vwoUserContext: WingifyUserContext?): String? {
    ...
    private suspend fun getAliasMappingFromServer(userId: String) =
        suspendCoroutine<Pair<Boolean, String>> { cont ->
            ...
            ioThreadAsync(callback = {
                ...
                response.data?.let { cont.resume(Pair(true, it)) } ?: run {
     ...
     */
    private suspend fun requestFromGatewayIfNotFoundLocally(vwoUserContext: WingifyUserContext): Boolean {
