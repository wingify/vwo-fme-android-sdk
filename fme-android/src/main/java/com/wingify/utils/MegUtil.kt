...
import com.wingify.utils.CampaignUtil.getBucketingSeed
...
class MegUtil {
    ...
    private fun isRolloutRuleForFeaturePassed(
        ...
        featureToSkip: MutableList<String?>, context: WingifyUserContext,
        storageService: StorageService,
        serviceContainer: ServiceContainer
    ): Boolean {
    ...
    private fun getEligibleCampaigns(
        ...
        context: WingifyUserContext, storageService: StorageService,
        serviceContainer: ServiceContainer
    ): Map<String, Any> {
    ...
    private fun findWinnerCampaignAmongEligibleCampaigns(
        ...
        groupId: Int, context: WingifyUserContext, storageService: StorageService,
        serviceContainer: ServiceContainer
    ): Variation? {
            ...
            val winnerVariation: Variation? =
                CampaignDecisionService(serviceContainer).getVariation(
                    ...
                    DecisionMaker().calculateBucketValue(
                        getBucketingSeed(bucketingId, null, groupId)
                    )
                )
            ...
            if (winnerCampaign == null) {
                ...
                for (campaign in shortlistedCampaigns) {
                    ...
                    if (wt!!.containsKey(campaignId.toString())) {
                        val clonedCampaign = FunctionUtil.cloneObject(campaign) as Campaign?
